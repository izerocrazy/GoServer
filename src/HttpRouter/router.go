...
*/
func (h *HttpRouter) AddControl(szPath string, control interface{}) string {
	var err string
	if h.Map == nil {
		err = "uninit"
	} else {
...
// 错误有：
// 对象错误：1、map 未初始化
// 参数错误：1、ResponseWriter 的错误；3、Request 的错误
*/
func (h *HttpRouter) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if h.Map == nil {
		fmt.Println("HttpRouter ServeHTTP Error: HttpRouter.ReflectMap is un Init")
	}

	// 从 map 中取出一个对象
	err, _ := h.Map.New(r.URL.Path)
	if err != "success" {
		fmt.Println("HttpRouter ServeHTTP Error: Map New a control err", err)
	}
